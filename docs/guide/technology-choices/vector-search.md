...
Azure Cosmos DB for NoSQL, Azure DocumentDB, Azure Database for PostgreSQL, and Azure SQL Database can store embeddings with the operational records that they describe. If your application generates an embedding and writes it with the record, both values are updated through the database write path. This design avoids a separate search-index synchronization process.
...
In Azure Cosmos DB for NoSQL, add supported predicates to the `WHERE` clause of a query that uses `VectorDistance`. A partition-key filter limits the search to the matching logical partition. In Azure DocumentDB, add supported filters to the `cosmosSearch` operator. In AI Search, create filterable non-vector fields and use [vector filter modes](/azure/search/vector-search-filters) to control whether the service applies a filter before or after vector retrieval.

Test approximate search with representative filters. A selective filter can reduce the number of eligible neighbors. Where the service provides exhaustive search, use it as a baseline when you measure recall.
...
Regional design also differs. Azure Cosmos DB provides built-in data distribution across Azure regions and supports multiple write regions. AI Search is a regional service. A multiregion AI Search design uses a search service in each region and requires a process that keeps the indexes aligned. Check current regional availability and service-level agreements before you select a topology.
...
Plan model migrations before the initial load. You can replace vectors in place, write a parallel vector field or index, or version the collection and switch clients after reprocessing completes. The migration cost depends on corpus size and embedding generation, not only query volume.

### Combine services when requirements differ

The services aren't mutually exclusive. An operational database can hold application state, conversation history, and frequently updated records while AI Search provides retrieval across enterprise content from multiple systems. Treat each retrieval path as a separate workload. Combine services when one path requires transactional freshness and another requires managed content processing or search-experience features.
